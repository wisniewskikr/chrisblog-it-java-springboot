...
##### Goal
The goal of this project is to present how to use class **CompletableFuture** and method **supplyAsync()** in **inner** method of FE application calling BE application in chain of applications (services). Back-End application **works 3 seconds per call**. Front-End application calls **3 times** Back-End application in one inner method. So with multithread expected result of method call is about **3 seconds**.

CompletableFuture.supplyAsync() starts new thread in asynchronous mode. It returns some value.
...
##### Required steps:
1. In the first command line tool **start Back-End application** with `mvn -f ./springboot-threads-inner-completablefuture-supplyasync_BE spring-boot:run`
1. In the second command line tool **start Front-End application** with `mvn -f ./springboot-threads-inner-completablefuture-supplyasync_FE spring-boot:run`
