...
** Kubernetes Secret** is an API object used to store confidential data in key-value pairs. Pods can consume Secrets as environment variables, command-line arguments, or as configuration files in a volume.
...
* **Download** source code and open any **Command Line** tool on **project's folder**. You can do it in following way:
    ...
    * Go to source code folder with `cd chrisblog-it\java-orchestration-kubernetes-minikube\springboot-kubernetes-secret-environment-variables`
...
1. (Optional) Check Kubernetes Secret

    * Display list of Kubernetes Secret with `kubectl get secrets`
    * Display describe for specific Kubernetes Secret with **kubectl describe secret {secret-name}**. For instance with `kubectl describe secret helloworld-secret` 
...
1. Clean up environment:
    ...
    * Delete Kubernetes ConfigMap with **kubectl delete configmap {configmap_name}** . For instance with `kubectl delete secret helloworld-secret`
