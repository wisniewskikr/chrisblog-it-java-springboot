...
Server application requires Client Authentication. In other words Server wants to be sure that communicates with trusted Client. That's why Client has to identify itself by API Key type Bearer - provides in HTTP header:
* **Key**: Authorization
* **Value**: Bearer {Token}
...
* **Download** source code and open any **Command Line** tool on **project's folder**. You can do it in following way:
    ...
    * Go to source code folder with `cd chrisblog-it\java-springboot-security-client-authentication\springboot-security-api-client-authentication-api-key-bearer`
...
1. Use Postman GET method (Bearer Token: "MyToken") with `http://localhost:9090`
...
Usage steps:
1. In the Command Line tool start application with `mvn -f ./springboot-helloworld-browser-json-ports-text spring-boot:run`
1. Use Postman GET method (Header: "Authorization / Bearer MyToken") with `http://localhost:9090`
