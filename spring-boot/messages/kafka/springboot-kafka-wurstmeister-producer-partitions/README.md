...
Additionally this project shows how to **configure many Partitions**. Partition is something like "subfolder" for Topic "folder". Goal of Partitions is to speed up performance of Kafka. Partitions for one Topic can be stored in many Kafka Clusters/Brokers. Topic with the same "key" should be stored in the same partition. It is configured in Producer's **application.yml** file. 
